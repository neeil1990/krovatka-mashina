				...
				<?php if( $total_price > 0 ) : ?>
					<div class="total-price">
						<?php
							...
							$total_price = sprintf( __( '%s за %s товар(а)', 'electro' ), $total_price_html, $total_products_html );
							echo wp_kses_post( $total_price );
						?>
					</div>
					<?php if( electro_get_shop_catalog_mode() == false ) : ?>
					<div class="accessories-add-all-to-cart">
						<button type="button" class="button btn btn-primary add-all-to-cart"><?php echo esc_html__( 'Добавить в корзину', 'electro' ); ?></button>
					</div>
					<?php endif; ?>
				<?php endif; ?>
			</div>
