...
if ( ! function_exists( 'electro_comment_meta' ) ) {
	...
	function electro_comment_meta() {
		if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) : ?>
			<span class="comments-link"><?php comments_popup_link( esc_html__( 'Оставить комментарий', 'electro' ), '1', '%' ); ?></span>
		<?php endif;
	}
}
...
if( ! function_exists( 'electro_post_readmore' ) ) {
	...
	function electro_post_readmore() {
		?>
		<div class="post-readmore"><a href="<?php the_permalink(); ?>" class="btn btn-primary"><?php echo apply_filters( 'electro_blog_post_readmore_text', esc_html__( 'Читать далее', 'electro' ) ); ?></a></div>
		<?php
	}
}
