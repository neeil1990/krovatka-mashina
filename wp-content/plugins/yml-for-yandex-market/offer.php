...
function yfym_feed_header() {
 ...
 $shop_name = stripslashes(yfym_optionGET('yfym_shop_name'));
 $company_name = stripslashes(yfym_optionGET('yfym_company_name'));
 ...
 $res_home_url = apply_filters('yfym_home_url', $res_home_url);
 ...
 $rateCB = apply_filters('yfym_rateCB', $rateCB); /* с версии 2.3.1. */
 if ($rateCB == '') {
...
function yfym_unit($postId) {
 yfym_error_log('Стартовала yfym_unit. postId = '.$postId.'; Файл: offer.php; Строка: '.__LINE__, 0);	
			
 $cur_post = get_post($postId);
 if ($cur_post == null)	{yfym_error_log('Товар с postId = '.$postId.' пропущен т.к get_post вернула null; Файл: offer.php; Строка: '.__LINE__, 0); $result_yml = ''; return $result_yml;}
 $result_yml = ''; 
 $product = wc_get_product($postId);
 
 if ($product->is_type('grouped')) {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к сгруппированный; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 
 // что выгружать
 if ($product->is_type('variable')) {
	$yfym_whot_export = yfym_optionGET('yfym_whot_export');
	if ($yfym_whot_export == 'simple') {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к вариативный; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 }
 
 if (get_post_meta($postId, 'yfymp_removefromyml', true) == 'on')  {yfym_error_log('Товар с postId = '.$postId.' пропущен принудительно; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 ...
 if (get_post_meta($postId, 'yfym_individual_delivery', true) !== '') {	
	...
	if ($delivery == 'off') {$delivery = yfym_optionGET('yfym_delivery');}
 } else {
 ...
 */	
 ...
 $result_yml_name = apply_filters('yfym_change_name', $result_yml_name, get_the_id(), $product);
 ...
 if (!empty($description_yml)) {
	...
	$description_yml = apply_filters('yfym_description_filter', $description_yml, $postId, $product);
...
/* ? */	$uzhe_est[] = $termin->term_taxonomy_id;
	  ...
	  yfym_error_log('Warning: Для товара $postId = '.$postId.' get_the_terms = false. Возможно база битая. Пробуем задействовать wp_get_post_terms; Файл: offer.php; Строка: '.__LINE__, 0);$product_cats = wp_get_post_terms($postId, 'product_cat', array("fields" => "ids" ));	  
	  ...
	  if (is_array($product_cats) && count($product_cats)) {
		...
		yfym_error_log('Warning: Для товара $postId = '.$postId.' база наверняка битая. wp_get_post_terms вернула массив. $catpostid = '.$catpostid.'; Файл: offer.php; Строка: '.__LINE__, 0);
	  }
	 }
	}
 } else {	
...
/* ? */	$uzhe_est[] = $termin->term_taxonomy_id;
	 ...
	 yfym_error_log('Warning: Для товара $postId = '.$postId.' get_the_terms = false. Возможно база битая. Пробуем задействовать wp_get_post_terms; Файл: offer.php; Строка: '.__LINE__, 0);
 ...
 if ($result_yml_cat == '') {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к нет категорий; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 ...
 */			  
 ...
 if ($product->is_type('variable')) {
	yfym_error_log('У нас вариативный товар. Файл: offer.php; Строка: '.__LINE__, 0);	
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		$result_yml_name = apply_filters('yfym_variable_change_name', $result_yml_name, get_the_id(), $product, $offer);
		/* с версии 2.0.7 в фильтр добавлен параметр $product */
		...
		*/
		...
		if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('Вариация товара с postId = '.$postId.' пропущена т.к нет цены; Файл: offer.php; Строка: '.__LINE__, 0); continue;}
		...
		if ($yfym_skip_missing_products == 'on') {
			if ($offer->is_in_stock() == false) {yfym_error_log('Вариация товара с postId = '.$postId.' пропущена т.к ее нет в наличии; Файл: offer.php; Строка: '.__LINE__, 0); continue;}
		}
		...
		if ($skip_backorders_products == 'on') {
		 if ($offer->get_manage_stock() == true) { // включено управление запасом			  
			if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {yfym_error_log('Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ и включено управление запасом; Файл: offer.php; Строка: '.__LINE__, 0); continue;}
		 }
		}
		...
		if (empty($thumb_yml)) {			
			...
			$no_default_png_products = yfym_optionGET('yfym_no_default_png_products');
		...
		$yfym_skip_products_without_pic = yfym_optionGET('yfym_skip_products_without_pic'); 
		if (($yfym_skip_products_without_pic == 'on') && ($picture_yml == '')) {	  
			yfym_error_log('Вариация товара с postId = '.$postId.' пропущена т.к нет картинки даже в галерее; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/  
		}
		...
		if (empty($no_group_id_arr)) {
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($param_at_name == '') {
					yfym_error_log('Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: functions.php; Строка: '.__LINE__, 0);	
					return $result_yml;	/*continue;*/
				}
				$result_yml_name_itog = $result_yml_name.' '.$so.$param_at_name.$sz;
				$result_yml_name_itog = apply_filters('yfym_name_no_groupid_filter', $result_yml_name_itog, $result_yml_name, $product, $so, $sz, $sd, $sr);
			} else {
		...
		/* с версии 2.1.2 */
		$append_offer_tag = apply_filters('yfym_append_offer_tag_filter', $append_offer_tag, $product);		
		...
		if (!empty($description_yml)) {
			...
			$description_yml = str_replace('<br>', '<br/>', $description_yml);
			$description_yml = strip_shortcodes($description_yml);
			$description_yml = apply_filters('yfym_description_filter', $description_yml, $postId, $product, $offer);
		...
		$result_url = apply_filters('yfym_url_filter', $result_url, $product, $CurCategoryId);
		...
		// }
		/* c версии 2.0.8 */
		$result_yml_pickup = apply_filters('yfym_pickup_filter', $result_yml_pickup, $catpostid, $product, $pickup);
		...
		/* с версии 2.3.0 */
		$stop_flag = false;
		$stop_flag = apply_filters('yfym_after_variable_offer_stop_flag', $stop_flag, $i, $variation_count, $offer_id, $offer);
 ...
 yfym_error_log('У нас обычный товар. Файл: offer.php; Строка: '.__LINE__, 0);
 // если цена не указана - пропускаем товар
 $price_yml = $product->get_price();
 if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к нет цены; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 if (class_exists('YmlforYandexMarketPro')) {
	if ((yfym_optionGET('yfymp_compare_value') !== false) && (yfym_optionGET('yfymp_compare_value') !== '')) {
		$yfymp_compare_value = yfym_optionGET('yfymp_compare_value');
		$yfymp_compare = yfym_optionGET('yfymp_compare');			 
 ...
 $yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products');
 if ($yfym_skip_missing_products == 'on') {
	if ($product->is_in_stock() == false) {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к нет в наличии; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml;}
 }		  
 ...
 if ($skip_backorders_products == 'on') {
	if ($product->get_manage_stock() == true) { // включено управление запасом  
		if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ и включено управление запасом; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	} else {
		if ($product->get_stock_status() !== 'instock') {yfym_error_log('Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	}
 }  
 ...
 $yfym_skip_products_without_pic = yfym_optionGET('yfym_skip_products_without_pic'); 
 if (($yfym_skip_products_without_pic == 'on') && ($picture_yml == '')) {	  
	yfym_error_log('Товар с postId = '.$postId.' пропущен т.к нет картинки даже в галерее; Файл: offer.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/  
 }
